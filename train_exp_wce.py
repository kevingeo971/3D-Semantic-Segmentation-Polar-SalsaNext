...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    handlers=[
                        logging.FileHandler("exp_wce.log"),
...
def main(args):
    ...
    weights = torch.tensor([22.9317, 857.5627, 715.1100, 315.9618, 356.2452, 747.6170, 887.2239, 963.8915,  5.0051,  63.6247,   
                            6.9002, 203.8796,   7.4802, 13.6315,   3.7339, 142.1462,  12.6355, 259.3699, 618.9667]).cuda()
    print(weights)
    loss_fun = torch.nn.CrossEntropyLoss(weight = weights, ignore_index=255)
            ...
            if global_iter % check_iter == 0:
                ...
                with torch.no_grad():
                    for i_iter_val,(_,val_vox_label,val_grid,val_pt_labs,val_pt_fea) in enumerate(val_dataset_loader):
                        ...
                        # l1 = lovasz_softmax(torch.nn.functional.softmax(predict_labels).detach(), val_label_tensor,ignore=255)
                        # l2 = loss_fun(predict_labels.detach(),val_label_tensor)
                        # print("l1,l2 ",l1,l2)
                        loss = loss = lovasz_softmax(torch.nn.functional.softmax(predict_labels).detach(), val_label_tensor,ignore=255) + loss_fun(predict_labels.detach(),val_label_tensor)
                        # print("**Total :",loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--model_save_path', default='./SemKITTI_PolarSeg_wce.pt')
